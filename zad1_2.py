...
class Language:
    ...
    def gather_aditional_info(self):
        for res in DDGS().text(f'{self.name} programming language', max_results=1):
            self.link = res["href"]
            self.priv_link = f"op_{self.name}.md"

    def __str__(self) -> str:
        return f"2024 position: {self.pos2024} Language: {self.name}, rating: {self.rating} (diff: {self.diff}), 2023 position: {self.pos2023}<br> <a href={self.link}> Click this note to learn more about {self.name}</a> <br> <a href={self.priv_link}>Author's opinion about {self.name}</a> <br><br>"
